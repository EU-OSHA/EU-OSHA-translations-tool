...
class MultipleTargetLanguageJobForm extends TmgmtFormBase {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['language_info']['show_target_languages'] = [
      '#title' => $this->t('Target languages') . ' (' . count($targetLanguagesCodes) . ')',
      '#type' => 'checkboxes',
    ...
    $form['translator_wrapper'] = [
      '#type' => 'details',
      '#title' => $this->t('Translator information'),
      '#open' => TRUE,
      '#weight' => 20,
    ];
    ...
    if ($messagesView = Views::getView('tmgmt_job_messages')) {
      ...
      // $messagesView->setDisplay('embed');
      // $messagesView->preExecute([$job->id()]);
      // $messagesView->execute();
      // foreach ($messagesView->result as $row) {
      //   foreach ($messagesView->field as $id => $field) {
      //     try {
      //       $rendered = $field->advancedRender($row);
      //     }
      //     catch (\Throwable $e) {
      //       \Drupal::logger('custom')->error('Error rendering field @field: @message', [
      //         '@field' => $id,
  ...
  public function checkoutInfo(JobInterface $job) {
    ...
    if (!$job->hasTranslator()) {
      return ['#markup' => $this->t('The job has no provider assigned.')];
    }
